...
from cumcm_b4.client import Position, SimulatorClient, probe, probe_interface  # noqa: E402
...
class TestDrillScriptPlumbing(unittest.TestCase):
    ...
    def test_drill_runs_end_to_end_offline(self):
        ...
        import cumcm_b4.client as client_mod
        import cumcm_b4.backend_http as http_mod
        ...
        try:
            ...
            sys.argv = ["run_drill.py", "--yes", "--tag", "selftest",
                        "--sim-data-dir", str(tmp / "no-such-dir")]
            buf = io.StringIO()
...
class TestDrillTruthReader(unittest.TestCase):
    ...
    def test_reads_newest_result_file(self):
        ...
        (tmp / "practice-p4-1-AAAA-AAAA-AAAA-AAAA.result.json").write_text(
            json.dumps({"version": 2, "problem_no": 4, "jammer_count": 16,
                        "omnidirectional_jammer_count": 9,
                        "directional_jammer_count": 7}), encoding="utf-8")
        payload, name = mod.read_practice_truth(str(tmp), time.time() - 600, timeout_s=2.0)
        self.assertEqual(payload["jammer_count"], 16)
        self.assertEqual(payload["directional_jammer_count"], 7)
        self.assertTrue(name.endswith(".result.json"))
        self.assertEqual(payload["problem_no"], 4)

    def test_ignores_stale_files_and_times_out_quietly(self):
        mod = self._load()
        tmp = Path(tempfile.mkdtemp(prefix="truth_stale_"))
        old = tmp / "practice-p4-2-BBBB-BBBB-BBBB-BBBB.result.json"
        old.write_text(json.dumps({"jammer_count": 12}), encoding="utf-8")
        payload, name = mod.read_practice_truth(str(tmp), time.time() + 3600, timeout_s=1.0)
    ...
    def test_missing_directory_returns_immediately(self):
        mod = self._load()
        t0 = time.time()
        payload, _ = mod.read_practice_truth(str(Path(tempfile.gettempdir()) / "nope-xyz"), 0.0, timeout_s=5.0)
        self.assertIsNone(payload)
...
class TestDrillParameterOverrides(unittest.TestCase):
    ...
    def test_infeasible_config_gives_readable_error(self):
        # 外环半径收到区域内部 → 包围网覆盖失败，应被拒绝
        bad_cfg = Path(tempfile.mkdtemp(prefix="badcfg_")) / "bad.json"
        bad_cfg.write_text(json.dumps({"scan_outer_radius_m": 1700.0}), encoding="utf-8")
