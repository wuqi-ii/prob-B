...
def _constraint_curves(config):
    ...
    """
    L, R, h, t = config.max_reception_m, config.guaranteed_reception_m, config.h, config.t
...
def plot_constraint_region(path, config, summary, poly, poly_full):
    ...
    ax.plot([0, config.max_reception_m], [0, 0], "--", color="#d1495b", lw=1.4,
    ...
    anchors = [(30.0, -905.0, (6, 0)),                      # (i) on the a=0 line
               ...
               (1500 + 1000 * math.cos(2.24), -config.h + 1000 * math.sin(2.24), (10, 6)),
               (430.0, config.t * 430.0, (-14, 8)),         # (iv) near angle line
               (995.0, config.t * (config.max_reception_m - 995) + config.h, (8, 6))]
    ...
    ax.set_xlim(-80, config.max_reception_m * 0.72)
    ...
    ax.set_title("第二问：五条硬约束如何截出固定候选区域\n"
                 f"（h=L·tan1°={config.h:.2f} m，t=tan31°={config.t:.4f}，R=1000 m，L=1500 m）")
    fig.tight_layout()
