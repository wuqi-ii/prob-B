...
def triangle(config):
    return [(0.0, 0.0), (config.far, -config.h), (config.far, config.h)]


def source_polygon(config):
    poly = clip_disk_outer(triangle(config), (0.0, 0.0), config.far, config.circle_sides)
...
def margins(point, config):
    ...
    return np.array([
        ...
        config.guaranteed_reception_m - math.hypot(x - config.far, y + config.h),
        y - config.t * x,
        y - config.t * (config.far - x) - config.h,
    ])
...
def feasible_mask(points, config, tol=1e-8):
    ...
    return ((x >= -tol)
            ...
            & (np.hypot(x - config.far, y + config.h) <= config.guaranteed_reception_m + tol)
            & (y >= config.t * x - tol)
            & (y >= config.t * (config.far - x) + config.h - tol))


def lobe_bounds(x, config):
    """Lower and upper |y| bounds at x, for an exact analytic region boundary."""
    x = np.asarray(x, dtype=float)
    low = np.maximum(config.t * x, config.t * (config.far - x) + config.h)
    R = config.guaranteed_reception_m
    r1, r2 = R * R - x * x, R * R - (x - config.far) ** 2
