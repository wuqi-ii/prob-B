...
from cumcm_b4.metrics import (  # noqa: E402
...
def run_one(seed: int, cfg: StrategyConfig, verbose: bool = False,
            n_sources: int | None = None, directional_prob: float = 0.5):
    case = generate_case(seed, n_sources=n_sources, directional_prob=directional_prob)
    ...
    detail = {
        ...
        "truth": [
            {"channel": s.channel, "x": round(s.x, 2), "y": round(s.y, 2),
             "radius": round(s.receiver_radius_m, 1),
             "directional": s.directional,
             "direction_deg": round(s.direction_deg, 1) if s.directional else None,
             "cleared": s.cleared}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="问题 4 离线演练测试")
