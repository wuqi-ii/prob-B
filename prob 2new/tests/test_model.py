...
def test_parameter_R_can_be_below_guaranteed_reception_radius():
    """R is a source-distance bound, not the receiver's physical radius."""
    c = Config(source_far_m=625.0, use_map_boundary=False)
    assert c.far == pytest.approx(625.0)
    assert c.h == pytest.approx(625.0 * math.tan(c.eps))
    tri = triangle(c)
    assert tri[1][0] == pytest.approx(625.0)
    assert station_grid(c).shape[0] > 0
    assert physical_checks(station_grid(c)[0], source_polygon(c), c)["candidate_feasible"]


def test_parameter_R_defaults_to_original_1500_model():
    original = Config()
    explicit = Config(source_far_m=1500.0)
    assert original.far == explicit.far == 1500.0
    assert np.array_equal(station_grid(original), station_grid(explicit))


def test_parameter_R_cannot_exceed_physical_maximum():
    with pytest.raises(ValueError):
        Config(source_far_m=1500.1)
